...
while true 
    ...
    pause(0.05); % Helps the UI actually run. Lower means faster
    if curr_key_count == 1; stopwatch_tic = tic; end;
...
%         fprintf("Skipping Check, no key pressed. curr_key_count = %d\n", curr_key_count);
    elseif keybot.keyCounter > curr_key_count %&& keybot.timerActive == 1
        ...
        if backspaces_needed > 0
            tempcmd = sprintf("connectedTeensy.writeline('S_K backspace');"); %TBD NEED CODE FOR BACKSPACE
